...
Matrix gp2sp(Matrix gp, Matrix v) {
  var x = gp[0];
  var y = gp[1];
  double a = v[0][0];
  double b = v[0][1];  
  ...
  var r1 = x*a + y*b + c;
  var r2 = x*d + y*e + f;
  return Matrix.fromRows([r1,r2],dtype: DType.float64);
}

Matrix solve(Matrix sp, Matrix gp) {
  var x = gp[0];
  var y = gp[1];
  var xp = sp[0];
  var yp = sp[1];
  ...
  return Matrix.fromRows([Cx,Cy],dtype: DType.float64);
}
